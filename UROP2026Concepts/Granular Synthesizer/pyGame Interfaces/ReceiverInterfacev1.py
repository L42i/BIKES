...
# --- OSC SENDER (To local SuperCollider) ---
...
# --- OSC RECEIVER (From remote SuperCollider via Network) ---
def remote_chord_handler(address, root_name, scale_name, degree):
    """This function is triggered automatically when a network message arrives."""
    global root, scale, active, root_open, scale_open
    try:
        root = NOTES.index(root_name)
        scale = scale_name
        active = degree - 1
        
        # Close dropdowns so the UI cleanly snaps to the new chord
        root_open = False
        scale_open = False
        print(f"📥 Remote Update: {root_name} {scale_name} {degree}")
    except ValueError:
        print("Received unknown note or scale.")

# Set up the receiver to listen on all IP addresses (0.0.0.0) on port 57121
disp = dispatcher.Dispatcher()
disp.map("/remoteChord", remote_chord_handler)
server = osc_server.ThreadingOSCUDPServer(("0.0.0.0", 57121), disp)

# Run the receiver in a background thread so it doesn't freeze PyGame
